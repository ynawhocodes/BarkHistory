...
router.post('/create_process', function(request, response) {
    db.query('SELECT * FROM post where category_id=?', [request.params.category_id],
        function(error, result) {
...
router.get('/:category_id', (request, response) => {
    db.query(
        "select post_id, post_title, post_detail, user_name,category_id,date_format(post_date, '%m/%d') as post_date, post_emotion_number from post left join user on post.user_id = user.user_id where category_id=? order by post_date desc", [request.params.category_id],
        function(error, posts) {
            if (error) {
                console.log(error);
            }
            db.query( //카테고리 이름, 설명 가져오기
                "select category_name, category_detail from category left join post on category.category_id=post.category_id where category.category_id=? limit 1", [request.params.category_id],
                function(error, data) {
                    if (error) {
                        console.log(error);
                    }
                    db.query( //실시간 인기글 가져오기 
                        'select post_title, post_detail, post_date from post where post_date between date_add(now(), interval -1 week)and now() and category_id=? order by post_emotion_number desc limit 1', [request.params.category_id],
...
router.post('/:category_id/:postId/:commentId/comment_update_process', function(request, response) {
    ...
    request.on('end', function() {
        var comment = qs.parse(body);
        console.log(comment);
        db.query(`UPDATE comment SET comment_detail=?, emotion_id=? WHERE comment_id=?`, [comment.comment, 1, comment.id],
            function(error, result) {
                if (error) {
                    throw error;
                }
